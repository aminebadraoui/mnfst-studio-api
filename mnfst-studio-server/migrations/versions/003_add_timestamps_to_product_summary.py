"""add timestamps to product summary

Revision ID: 003_timestamps
Revises: 002_update_product_summary
Create Date: 2024-01-31
"""
from alembic import op

# revision identifiers, used by Alembic.
revision = '003_timestamps'
down_revision = '002_update_product_summary'
branch_labels = None
depends_on = None
    ...
    op.execute("""
        ...
        BEGIN
            ...
            ELSIF (TG_OP = 'UPDATE') THEN
                -- Update product if any field changed
                            ...
                            SELECT jsonb_agg(
                                CASE 
                                    ...
                                    THEN jsonb_build_object(
                                        ...
                                        'name', NEW.name,
                                        'description', NEW.description,
                                        'created_at', COALESCE(NEW.created_at, OLD.created_at, NOW()),
                                        'updated_at', COALESCE(NEW.updated_at, NOW())
                                    )
                                    ELSE value 
                                END
...
def downgrade():
    ...
    # Recreate previous version of functions
    op.execute("""
        CREATE OR REPLACE FUNCTION update_project_products_summary()
        RETURNS TRIGGER AS $$
        BEGIN
